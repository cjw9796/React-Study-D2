...
1. useRef
    ...
    => Ref의 값들은 아무리 변경해도 컴포넌트를 다시 렌더링 되지 않는다. 불필요한 렌더링을 막을 수 있다.
        ...
        DOM 요소에 접근 
        예를 들어 로그인 화면이 보여줬을 때 id를 넣는 input을 굳이 클릭하지 않아도 자동적으로 포커스가 되어 있게 해주면 바로 키보드를 입력해서
        id를 입력가능 
...
export default function Use(){
    {/*
    각각의 Ref Up 버튼과 Var Up 버튼을 클릭하면 콘솔의 값이 증가하는 것을 볼 수있다.
    Render 버튼을 클릭하지 않으면 화면에 업데이트 되지 않는다. 

    화면에서 Ref의 값이 7이고, Var의 값이 5일 때 Render 버튼을 클릭했을 때, 
    Ref는 7이 표시되지만 Var은 0으로 표시된다. 
    왜냐하면 렌더링이 될 때마다 컴포넌트 함수 내부에 있는 변수들이 초기화되지만 // 
    Ref의 값은 컴포넌트의 전 생애주기를 통해 유지되는데 
    컴포넌트가 브라우저의 마운팅 된 시점부터 마운트가 해제될 때까지 같은 값을 계속 유지할 수 있기 때문이다. 
    마운트 : 연결시켜주는 과정 
    컴포넌트 생애주기 : 생성(mounting) -> 업데이트(updating) -> 제거(unmounting) 

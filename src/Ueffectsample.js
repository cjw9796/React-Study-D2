...
export default function Ueffectsample(){
    ...
    //1. Mount 탄생 시점
    // 버튼은 계속 눌리지만 콘솔에서는 한번만 실행이 되는 것을 확인할 수 있다. 
    //업데이트는 누르는 만큼 업데이트 된다. 
    // 컴포넌트가 탄생(mount) 시점에 콘솔이 실행되고 =>console.log("생성(Mount) 탄생했습니다.")
    // 다른 값을 넣었을 때 (클릭이나 Input 값 넣었을 때) 확인해보면 콘솔에는 한번만 확인할 수 있다. 

    useEffect(() =>{
        console.log("생성(Mount) 탄생했습니다.")
    },[]);
    ...
    return(
        <div>
             ...
             * onClick 이나 onChange 안에 바로 넣을 수 있다. 
             */}
